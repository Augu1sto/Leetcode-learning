...
两种情况下，都有五个连续的 'T' 。
 ...
 Related Topics 字符串 二分查找 前缀和 滑动窗口 👍 124 👎 0
...
class MaximizeTheConfusionOfAnExam{
    ...
    class Solution {
        public int maxConsecutiveAnswers(String answerKey, int k) {
            return Math.max(getMaxCnt(answerKey,k,'T'), getMaxCnt(answerKey,k,'F'));
        }

        int getMaxCnt(String answerKey, int k, char ch) {
            int i = 0;
            int cnt = 0; // 数滑动区间内ch的个数
            int ans = 0;
            for (int j = 0; j < answerKey.length(); j++) {
                if (ch == answerKey.charAt(j)) {
                    cnt++;
                }
                while(cnt>k){
