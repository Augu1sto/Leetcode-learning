...
： 
 ...
 否则，将请求安排给下一个空闲的服务器（服务器构成一个环，必要的话可能从第 0 个服务器开始继续找下一个空闲的服务器）。比方说，如果第 i 个服务器在忙，那么会
查看第 (i+1) 个服务器，第 (i+2) 个服务器等等。 
...
输出：[0]
 ...
 Related Topics 贪心 数组 有序集合 堆（优先队列） 👍 70 👎 0
...
class FindServersThatHandledMostNumberOfRequests{
    public static void main(String[] args) {
        ...
        // TO TEST
        List<Integer> test = solution.busiestServers(3,new int[]{1,2,3,4,5},new int[]{5,2,3,3,3});
    }
    ...
    class Server{
        ...
        Server(int _id){
            id = _id;
            this.available_time = 1;
        }

        boolean isAvailable(int curr){
            if (this.available_time <= curr) {
                return true;
            } else {
                return false;
            }
        ...
        void use(int curr, int time){
            this.available_time = curr+time;
        }
